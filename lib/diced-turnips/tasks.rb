...
namespace :parallel do
  ...
  task "turnips", %i[ count pattern options ] do |t, args|
    ...
    opts = %w[ -n --pattern --test-options ].zip(DicedTurnips::Tasks.parse_args(args)).reject do |_, v|
      "#{v}".empty?
    end

    executable = File.join(File.dirname(__FILE__), "..", "..", "bin", "parallel_turnip")

    command = [ executable, "spec", *opts.flatten ]
